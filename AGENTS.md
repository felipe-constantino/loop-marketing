...
1. **ANTES de começar**: verificar se existe arquivo de projeto ativo em `.Codex/loop-marketing/`. Checar primeiro se há um `.Codex/loop-marketing/_active.md` — ele aponta para o projeto atual. Se não houver `_active.md`, listar os arquivos em `.Codex/loop-marketing/` e perguntar qual projeto usar. Ler o arquivo do projeto e usar como contexto.
...
2. **Verificar credenciais de CRM/API na memória do projeto**: se houver token de API, pipeline IDs ou credenciais salvas nos arquivos de memória (`.Codex/loop-marketing/memory/`), usá-los para puxar dados diretamente antes de perguntar ao usuário.
